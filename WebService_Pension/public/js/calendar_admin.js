function getPensionName(room_num) {
    switch(room_num) {
        case 1:
            return '행복';
...
function calendar(mon,year,_reserv_)
{
    ...
    var next_date_query = "/Admin?mon=" + next_date.getMonth() + "&year=" + (next_date.getYear()+1900);
    var prev_date = new Date(date);
    prev_date.setMonth(prev_date.getMonth()-1);
    var prev_date_query = "/Admin?mon=" + prev_date.getMonth() + "&year=" + (prev_date.getYear()+1900);
    ...
    for(i=1;i<=total;i++)
    {
        ...
        var sDate = year +  "-" +  mon + "-" + i;

        var addStr = "<br/>";
        if(reserv_data[i] != null){
            for( ri = 0 ; ri < reserv_check.length ; ++ri){ reserv_check[ri] = 0; reserv_sn[ri] = 0; }
            for( rj = 0 ; rj < reserv_data[i].reserv_room.length ; ++rj){
                reserv_check[ reserv_data[i].reserv_room[rj].room_num -1 ] = 1;
                reserv_data[ reserv_data[i].reserv_room[rj].room_num -1 ] = { sn: reserv_data[i].reserv_room[rj].sn, name: reserv_data[i].reserv_room[rj].name, tel: reserv_data[i].reserv_room[rj].tel };
            }

            for(j = 0 ; j < room_name.length ; ++j) {

                if(reserv_check[j] == 1){
                    addStr += (room_name[j] + '<strong style="color:red;"><div style="display:inline;" class="calendar_btn" data-room='
                    +(j+1)+' data-date="'+sDate+'" data-action="cancel" data-sn="'+reserv_data[j].sn+'" data-name="'+reserv_data[j].name+'" data-tel="'+reserv_data[j].tel+'"> 예약완료</div></strong><br/>');
                }
                else{
                    addStr += (room_name[j] + '<strong> <div style="display:inline;" class="calendar_btn" data-room='
                    +(j+1)+' data-date="'+sDate+'" data-action="reserv">예약가능</div></strong><br/>');
                }
            }
        }
        else {
